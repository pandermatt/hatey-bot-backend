...
class ToxicCommentClassifier:
    ...
    def validation(self, val_dataloader):
        ...
        logit_preds, true_labels, pred_labels, tokenized_texts = self._training_loop(val_dataloader)
        self._calculate_accuracy(pred_labels, true_labels)

    def test(self, test_dataloader):
        self.model.eval()

        logit_preds, true_labels, pred_labels, tokenized_texts = self._training_loop(test_dataloader)
        true_bools, pred_bools = self._calculate_accuracy(pred_labels, true_labels)

        clf_report = classification_report(true_bools, pred_bools, target_names=self.label_columns)
        print(clf_report)

    def _training_loop(self, val_dataloader):
        ...
        return logit_preds, true_labels, pred_labels, tokenized_texts

    def _calculate_accuracy(self, pred_labels, true_labels):
        pred_labels = [item for sublist in pred_labels for item in sublist]
        true_labels = [item for sublist in true_labels for item in sublist]
        true_bools = [tl == 1 for tl in true_labels]
        pred_bools = [pl > self.threshold for pl in pred_labels]
    ...
    def save_model(self, name):
        torch.save(self.model, config.result_file(name))

    def load_model(self, name):
        self.model = torch.load(config.result_file(name))
